...
- Limit the path to the type of file you want. This usually means:
  - `path:meta.yaml` for v0 recipes.
  - `path:recipe.yaml` for v1 recipes.
...
- [All `noarch: python` v0 recipes](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Ameta.yaml+%22noarch%3A+python%22).
- [All `noarch: python` v1 recipes](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Arecipe.yaml+%22noarch%3A+python%22).
- Recipes that depend on... [`cuda`](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Ameta.yaml+OR+path%3Arecipe.yaml+%22-+cuda%22), [`pytorch`](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Ameta.yaml+OR+path%3Arecipe.yaml+%22-+pytorch%22), [`__virtual` packages](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Ameta.yaml+OR+path%3Arecipe.yaml+%2F-+__%5CS%2B%2F), etc.
...
- [v0 recipes with cross-compilation conditions](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Ameta.yaml+%22target_platform+%21%3D+build_platform%22+OR+%22build_platform+%21%3D+target_platform%22).
- [v1 recipes with cross-compilation conditions](https://github.com/search?type=code&q=org%3Aconda-forge+path%3Arecipe.yaml+build_platform).
...
Please refer to [How-to > Basics > Use CMake](/docs/how-to/basics/cmake/) and [How-to > Advanced > Move from autotools to CMake](/docs/how-to/advanced/autotools-to-cmake/).
...
Please refer to [How-to > Advanced > Windows > Building packages for Windows](/docs/how-to/advanced/windows/).
...
<!-- backwards compatiblity anchors -->
...
Here is an example part of the recipe:
...
context:
  build_base: 0
  build: ${{ build_base if mpi != 'nompi' else build_base + 100 }}
  # add build string so packages can depend on
  # mpi or nompi variants explicitly:
  # `pkg * mpi_mpich_*` for mpich
  # `pkg * mpi_*` for any mpi
...
Variant hash avoids build string collisions on _most_ variants,
...
```recipe
string: "${{ mpi_prefix }}_py${{ python | version_to_buildstring }}h${{ hash }}_${{ build }}"
```
...
runtime. MSVC may use its own runtime `vcomp14` or `llvm-openmp`. However, individual projects may force building
against a different runtime through their build system configuration.
...
In the recipe file, add the following to ensure that the user's system is compatible.
...
- Specify the environment variable `SETUPTOOLS_SCM_PRETEND_VERSION` with the version string.
  ...
  - If you are using build scripts, specify in the build script:
...
See [How to configure self-hosted runners](/docs/how-to/advanced/self-hosted-runners.mdx).
...
<a id="packages-without-upstream-versions"></a>

## Packages without upstream versions

The preferred approach is always to **ask upstream to add version tags** to
their repository. Proper upstream versioning benefits the entire packaging
ecosystem and avoids ambiguity.

However, when this is impractical — for example, when the upstream repository
only uses platform-specific release tags (e.g. Android release versions) that
don't reflect the package's own versioning, or the project is unlikely to
adopt conventional versioning — use the **`0.0.0.YYYY.MM.DD`** version format,
where `YYYY.MM.DD` is the date of the pinned commit.

This format is preferred because:

- **All segments are numeric**, which is required by conda's
  `apply_pin_expressions()` for compatibility specifications.
- **Any future real release** (e.g. `0.0.1`) will automatically sort higher.
- It clearly communicates that this is not an official upstream version.

When packaging from a specific commit, always pin the source using `rev:` with
the full commit hash rather than a branch name. This ensures reproducible
builds.

```recipe title="recipe.yaml"
context:
  version: "0.0.0.2026.01.22"

source:
  git: https://example.com/project.git
  rev: abc123def456  # pin to a specific commit
```

For more context, see the discussion on
[conda-forge/staged-recipes#30713](https://github.com/conda-forge/staged-recipes/pull/30713).
